...
### Added
- Tippy's own voice: every sentence the app can say in English (about 830, 2.7 MB) is now a recording made with the Piper "hfc_female" voice, pitched up to sound lighter and friendlier, instead of the computer's robotic voice. The child's own name and anything without a recording (lines written by the online helper, unusual typed words) are still said by the computer's voice. German (Thorsten, the livelier "surprised" recording) and Spanish from Spain (Sharvard, female, livelier settings) are recorded too, so they no longer depend on the computer having a German or Spanish voice (the computer often read German with an English voice, which sounded completely wrong). The English recordings are for non-commercial use (CC BY-NC-SA); the licences are in `frontend/voice/LICENSE.md`. Made with `scripts/make_voice.py` (a developer tool).
