...
### Added
- Twemoji pictures everywhere instead of the operating system's own emoji, so images look the same, colourful and complete on every computer (Windows was missing flag pictures entirely). A redrawn mascot with soft shading and paws. `scripts/fetch_emoji.py` (developer tool) fetches only the pictures Tippy actually uses.
