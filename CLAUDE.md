...
- Signing: the app is ad-hoc signed only. A Developer ID certificate and notarisation need the paid Apple Developer Program ($99/year); the owner has a free account, which cannot do either. Not done yet: code signing with an Apple Developer ID and notarisation (so other people's Macs do not warn), a universal (Intel + Apple silicon) build, a release-pipeline job that attaches the app, and a Windows installer.
